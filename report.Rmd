...
if(l){
 ...
 # p4 <- plot_ly(ds, x = ~ds$rowname, y = ~ds$random, key = ~key, type = 'scatter', mode='markers',  marker = list(color = 'blue', opacity=2))%>%layout(title = 'Random according to the word', xaxis = list(title ='Word'), yaxis =list(title ='Random'), titlefont = 'arial', showlegend = FALSE)
  #tmpFile <- tempfile(fileext = ".png")
